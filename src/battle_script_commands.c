...
static u32 GetTrainerMoneyToGive(u16 trainerId)
{
    ...
    u32 moneyReward;
    u8 lastMonLevel = 0;
    struct Trainer t = RedirectTrainer(trainerId);
    ...
    else
    {
        ...
        case 0:
            {
                const struct TrainerMonNoItemDefaultMoves *party = t.party.NoItemDefaultMoves;
                lastMonLevel = party[t.partySize - 1].lvl;
            }
            break;
        case F_TRAINER_PARTY_CUSTOM_MOVESET:
            {
                const struct TrainerMonNoItemCustomMoves *party = t.party.NoItemCustomMoves;
                lastMonLevel = party[t.partySize - 1].lvl;
            }
            break;
        case F_TRAINER_PARTY_HELD_ITEM:
            {
                const struct TrainerMonItemDefaultMoves *party = t.party.ItemDefaultMoves;
                lastMonLevel = party[t.partySize - 1].lvl;
            }
            break;
        case F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_HELD_ITEM:
            {
                const struct TrainerMonItemCustomMoves *party = t.party.ItemCustomMoves;
                lastMonLevel = party[t.partySize - 1].lvl;
            }
            break;
        }
        ...
        {
            if (gTrainerMoneyTable[i].classId == t.trainerClass)
                break;
        }
